...
export default function EntryPage() {
  ...
  useEffect(() => { if (getToken()) fetchEmployeeMe().then(async (user) => { if (!['employee', 'admin'].includes(user.role)) throw new Error(); await Taro.reLaunch({ url: '/pages/workbench/index' }) }).catch(() => { setToken(''); setError('登录状态已失效，请重新验证') }).finally(() => setLoading(false)) }, [])
  const login = async () => {
    if (!identifier.trim() || !password) { setError('请输入内部账号和密码'); return }
    setLoading(true); setError('')
    try { const session = await employeePasswordLogin(identifier, password); if (!['employee', 'admin'].includes(session.role || '')) throw new Error('该账号没有店员权限'); setToken(session.access_token); await Taro.reLaunch({ url: '/pages/workbench/index' }) }
    catch (error) { setError(error instanceof Error ? error.message : '验证失败，请稍后重试') }
