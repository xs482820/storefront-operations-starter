...
def _serialize_user(item: User) -> UserAdminOut:
    ...
    if item.profile:
        profile = CustomerProfileOut(
            ...
            miniapp_notification_enabled=bool(item.profile.miniapp_notification_enabled),
            miniapp_notification_event_keys=list(item.profile.miniapp_notification_event_keys or []),
            miniapp_notification_updated_at=(
                item.profile.miniapp_notification_updated_at.isoformat()
                if item.profile.miniapp_notification_updated_at
                else None
            ),
        )
    return UserAdminOut(
