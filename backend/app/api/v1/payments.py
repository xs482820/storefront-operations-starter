...
from app.services.orders import mark_order_paid, release_order_reservations
...
@router.post("/wechat/sync", response_model=PaymentRecordOut)
async def sync_wechat_payment(
    payload: PaymentSyncIn,
    db: Annotated[AsyncSession, Depends(get_db)],
    current_user: Annotated[User, Depends(get_current_user)],
) -> PaymentRecordOut:
    ...
    if settings.WECHAT_PAY_MOCK:
        if payment.status == PaymentStatus.PENDING:
            ...
            payment.provider_txn_no = payment.provider_txn_no or f"MOCK-{order.order_no}"
            payment.provider_payload = {
                **(payment.provider_payload or {}),
                "mock_sync": True,
            }
            await mark_order_paid(db=db, order=order, operator=current_user, source="customer", note="mock wechat payment sync")
            await db.commit()
        return _serialize_payment(payment, order.order_no)

    provider_data = await query_transaction_by_out_trade_no(order.order_no)
    trade_state = (provider_data.get("trade_state") or "").upper()
    target_status = _payment_status_from_trade_state(trade_state)
    provider_txn_no = provider_data.get("transaction_id")
    ...
    if target_status == PaymentStatus.PAID:
        payment.status = PaymentStatus.PAID
        payment.provider_txn_no = provider_txn_no or payment.provider_txn_no
        payment.provider_payload = provider_data
        await mark_order_paid(db=db, order=order, operator=current_user, source="customer", note="wechat payment sync")
        await db.commit()
...
async def update_payment_status(
    ...
    if payload.status == PaymentStatus.PAID and order.status == OrderStatus.PENDING_PAYMENT:
        await mark_order_paid(db=db, order=order, source="admin", note=payload.note or "admin payment status update")
...
async def wechat_v3_callback(req: Request, db: Annotated[AsyncSession, Depends(get_db)]) -> dict:
    ...
    try:
        ...
        logger.warning("wechat v3 callback signature verification failed: %s", error)
    ...
    try:
        ...
        logger.warning("wechat v3 callback resource decrypt failed: %s", error)
        raise
    ...
    if is_refund_callback:
        ...
        if target_status == PaymentStatus.PAID and order.status == OrderStatus.PENDING_PAYMENT:
            await mark_order_paid(db=db, order=order, source="wechat_callback", note="wechat payment callback")
        elif target_status == PaymentStatus.REFUNDED:
            if order.status != OrderStatus.CANCELED:
                await release_order_reservations(db=db, order=order)
            order.status = OrderStatus.CANCELED
