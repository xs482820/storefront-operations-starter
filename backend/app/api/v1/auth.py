...
async def _build_token_response(user: User, generated_password: str | None = None, is_new_user: bool = False, expires_minutes: int | None = None) -> TokenResponse:
    token = create_access_token(subject=user.username, expires_minutes=expires_minutes, session_version=user.session_version)
...
async def login(
    ...
    employee_expiry = get_settings().EMPLOYEE_JWT_EXPIRE_MINUTES if user.role == UserRole.EMPLOYEE else None
    return await _build_token_response(user, expires_minutes=employee_expiry)
...
async def request_phone_code(payload: PhoneCodeRequestIn) -> PhoneCodeRequestOut:
    ...
    return PhoneCodeRequestOut(
        ...
        # ponytail: debug codes are for local UI development only; production must never return a login secret.
        debug_code=code if settings.APP_DEBUG and settings.AUTH_SMS_MOCK else None,
    )
...
async def wechat_mini_code2session(payload: WechatMiniCodeIn) -> WechatMiniCodeOut:
    result = await code_to_session(payload.code, payload.app_scope)
...
async def wechat_mini_login_with_phone(
    ...
    session_result = await code_to_session(payload.login_code, payload.app_scope)
    openid = session_result["openid"]
    unionid = session_result.get("unionid")
    phone = normalize_phone(await get_phone_number(payload.phone_code, payload.app_scope))

    # Employee and customer miniapps have different openids. Employee access is
    # deliberately matched by the authorized phone, never by the customer binding.
    user = None if payload.app_scope == "employee" else await _find_user_by_wechat_identity(db, openid, unionid)
    is_new_user = False
